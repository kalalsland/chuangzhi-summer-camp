...
class MyHarness(Harness):
    ...
    def _build_messages(self, text: str):
        ...
        # LIKELY_CANDIDATES：few-shot 涉及的 label + 检索 top-10 补足，去重最多 8 个。
        # 不收紧 LABEL_SET（模型仍可选任意 label），只用作"高亮候选"信号——
        # 应对 hard-case 分析里"被吸光 label 抢走"的错误模式。
        likely, seen_lik = [], set()
        for d in chosen:
            lab = self.memory[d][1]
            if lab not in seen_lik:
                seen_lik.add(lab)
                likely.append(lab)
        for d in topk[:10]:
            if len(likely) >= 8:
        ...
        def build(labels_seq, ex_count, with_likely=True):
            parts = ["LABEL_SET: " + ", ".join(labels_seq)]
            if with_likely and likely:
                parts.append(
                    "LIKELY_CANDIDATES (most relevant from retrieval, in priority order): "
                    + ", ".join(likely)
                )
            if ex_count > 0:
        ...
        for lab, _ in self._label_count.most_common():
            if lab not in seen:
                ...
                chosen_labs.append(lab)
        while chosen_labs:
            msgs = build(chosen_labs, 0, with_likely=False)
            if self.count_messages_tokens(msgs) <= budget:
                return msgs
            chosen_labs.pop()
