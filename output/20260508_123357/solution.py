...
class MyHarness(Harness):
    def __init__(self, call_llm, count_tokens, count_messages_tokens, max_prompt_tokens: int):
        ...
        self._ood_threshold = None              # OOD 阈值（首次 predict 时从 train 自检索算 P5）
    ...
    def _search_full_ranked(self, text: str, k: int) -> list:
        """RRF 融合后完整 [(score, doc_idx), ...] 取前 k 个。"""
        if self._N == 0:
            return []
        M = max(k * 3, 60)
        ...
        for rank, (_, d) in enumerate(bm25_ranked):
            rrf[d] = rrf.get(d, 0.0) + 1.0 / (RRF_K + rank + 1)
        ranked = sorted(((s, d) for d, s in rrf.items()), reverse=True)
        return ranked[:k]

    def _search(self, text: str, k: int) -> list:
        """RRF 融合 TF-IDF cosine + BM25，返回 top-k doc_idx。"""
        return [d for _, d in self._search_full_ranked(text, k)]

    def _ensure_ood_threshold(self) -> None:
        """从 train 集 self-retrieval 的 top-1 RRF 分布推 P5 作为 OOD 阈值。"""
        if self._ood_threshold is not None:
            return
        if self._N < 10:
            self._ood_threshold = 0.0
            return
        self_scores = []
        for i in range(self._N):
            text = self.memory[i][0]
            ranked = self._search_full_ranked(text, 3)
            # 剥离自己，取最高的非自身分数
            best = 0.0
            for s, d in ranked:
                if d != i:
                    best = s
                    break
            self_scores.append(best)
        self_scores.sort()
        idx = max(0, int(self._N * 0.05) - 1)
        self._ood_threshold = self_scores[idx]
    ...
    def _build_messages(self, text: str):
        ...
        OOD 分支：若 top-1 RRF 分数低于 train 集自检索 P5 阈值 → 不带 few-shot。
        """
        # 检索 + per-label cap=2，最多 8 条
        ranked24 = self._search_full_ranked(text, 24)
        topk = [d for _, d in ranked24]
        top1_score = ranked24[0][0] if ranked24 else 0.0

        # 实验 C：OOD 检测
        self._ensure_ood_threshold()
        ood_mode = (self._ood_threshold is not None) and (top1_score < self._ood_threshold)
