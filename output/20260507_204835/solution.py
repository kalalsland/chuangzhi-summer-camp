...
class MyHarness(Harness):
    ...
    def _build_messages(self, text: str):
        ...
        system_msg = (
            ...
            "First write at most one short sentence of reasoning prefixed by 'Reason:'. "
            "Then on a NEW LINE output 'Label: <label>' where <label> is exactly one item from LABEL_SET. "
        ...
        def _clean(s):
            # 保留换行（选择题、多行注入文本结构不能扁平化）；仅去除 \r、统一行尾、剥首尾空白
            if not s:
                return ""
            s = s.replace("\r\n", "\n").replace("\r", "\n")
            return s.strip()
        ...
        # 3) 收缩 label：先保留检索涉及到的 label，再按训练频率补
        chosen_labs = []
        seen = set()
        for d in chosen:
            lab = self.memory[d][1]
    ...
    # 自一致性投票次数：N=3 试过对 DashScope qwen3-8b 无效（同 prompt 输出 deterministic），
    # 暂回退到 1；如服务端切到真采样可改大
    _N_VOTES = 1
    ...
    def predict(self, text: str) -> str:
        ...
        votes = []
        first_raw_label = None  # 平局时 fallback 用第一次回答
        for _ in range(self._N_VOTES):
            try:
                resp = self.call_llm(messages)
            except Exception:
                continue
            lab = self._parse_label(resp)
            if first_raw_label is None and lab:
                first_raw_label = lab
            if lab in self._label_set:
                votes.append(lab)

        if not votes:
            return first_raw_label or self._fallback_label()

        cnt = Counter(votes)
        top_count = cnt.most_common(1)[0][1]
        # 多数票；平局时优先选第一次回答（如果它在赢家集合里）
        winners = [lab for lab, c in cnt.items() if c == top_count]
        if len(winners) == 1:
            return winners[0]
        if first_raw_label in winners:
            return first_raw_label
        return winners[0]
