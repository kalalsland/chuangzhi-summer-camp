...
class MyHarness(Harness):
    def __init__(self, call_llm, count_tokens, count_messages_tokens, max_prompt_tokens: int):
        ...
        self._aug_done = False                  # 测试 II：LLM 标签样本增强是否已执行
    ...
    def _search(self, text: str, k: int) -> list:
        """[消融实验] 仅 BM25 排序，返回 top-k doc_idx。"""
        return [d for _, d in self._search_bm25_ranked(text, k)]
    ...
    # -------- 测试 II：LLM 标签样本增强 --------
    def _ensure_label_augmented(self) -> None:
        """首次 predict 时触发；对每 label 调 LLM 生成 5 条 paraphrase 扩充检索索引。"""
        if self._aug_done:
            return
        with self._aug_lock:
            if self._aug_done:
                return
            # 按 label 收集原始样本（通常每 label 3 条）
            label_samples = defaultdict(list)
            for text, label in self.memory:
                label_samples[label].append(text)

            # 分批（每批 4 个 label），控制单次 prompt 长度
            labels_sorted = list(self._labels)
            BATCH = 4
            GEN_PER_LABEL = 5
            for i in range(0, len(labels_sorted), BATCH):
                batch_labels = labels_sorted[i:i + BATCH]
                parts = [
                    "You generate paraphrased training samples for a text classification task.",
                    "For each label below, produce EXACTLY %d short paraphrase utterances that naturally belong to that label." % GEN_PER_LABEL,
                    "Each paraphrase must preserve the underlying intent but vary wording/phrasing.",
                ...
                for lab in batch_labels:
                    samples = label_samples.get(lab, [])[:3]
                    parts.append("Label: %s" % lab)
                    for j, s in enumerate(samples, 1):
                        cleaned = s.replace("\n", " ").strip()[:200]
                        parts.append("  ex%d: %s" % (j, cleaned))
                    parts.append("")
                parts.append("Now produce paraphrases for all %d labels above." % len(batch_labels))
                msgs = [
                    {"role": "system", "content": "You are a precise data augmentation assistant. Follow the output format exactly."},
                    {"role": "user", "content": "\n".join(parts)},
                ]
                try:
                    ...
                    continue
                self._ingest_aug_response(resp, set(batch_labels))
            self._aug_done = True

    def _ingest_aug_response(self, resp: str, valid_labels: set) -> None:
        """解析 <label=X>... </label> 块，把行号条目作为新样本 update 进来。"""
        if not resp:
            return
        blocks = re.findall(r"<label=([^>]+)>(.*?)</label>", resp, re.DOTALL | re.IGNORECASE)
        for lab_raw, body in blocks:
            lab = lab_raw.strip()
            ...
            for line in body.splitlines():
                ...
                if not m:
                    continue
                para = m.group(1).strip().strip('"').strip("'")
                if len(para) < 3:
                    continue
                # 走 update 标准通道：memory + 倒排 + TF + doc_len 自动维护
                self.update(para, lab)

    def predict(self, text: str) -> str:
        if not self._labels:
            return ""
        self._ensure_label_augmented()
