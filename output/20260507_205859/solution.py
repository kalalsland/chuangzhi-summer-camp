...
class MyHarness(Harness):
    ...
    def predict(self, text: str) -> str:
        ...
        # ===== Stage 1：让 LLM 在全 LABEL_SET 上给出 top-3 候选 =====
        s1_msgs = self._build_messages(
            text,
            system_msg=self._SYSTEM_TOPK,
            tail="Output three labels separated by ' | ', best first. Nothing else.",
        )
        s1_top = []
        try:
            resp1 = self.call_llm(s1_msgs)
            s1_top = self._parse_top_labels(resp1, max_k=5)
        except Exception:
            pass

        # ===== 合并 shortlist：stage1 top-3 ∪ 检索 top-K labels =====
        shortlist, seen = [], set()
        for lab in s1_top:
            if lab in self._label_set and lab not in seen:
                seen.add(lab)
                shortlist.append(lab)
        # 从检索补到 6 个
        topk_idx = self._search(text, k=24)
        for d in topk_idx:
            if len(shortlist) >= 6:
                break
            lab = self.memory[d][1]
        ...
        # 极端 fallback：shortlist 空 → 退到全 label
        if not shortlist:
            shortlist = list(self._labels)

        # ===== Stage 2：在 shortlist 内精选 =====
        s2_msgs = self._build_messages(text, labels_override=shortlist)
        try:
            resp2 = self.call_llm(s2_msgs)
        except Exception:
            return shortlist[0] if shortlist else self._fallback_label()
        return self._parse_label(resp2)
