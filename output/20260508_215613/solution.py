...
class MyHarness(Harness):
    def __init__(self, call_llm, count_tokens, count_messages_tokens, max_prompt_tokens: int):
        ...
        self._is_mcq_cache = None               # MCQ 检测结果缓存（基于 train text 选项 stem，独立于 desc 生成状态）
    ...
    def _retrieval_fallback(self, text: str) -> str:
        """LLM 不可用（429/超时/异常）时，用检索 top-1 的 label 作兜底。
        命中率约等于检索 top-1（实证 47.3%），远优于训练集最高频 label。"""
        if text:
            try:
                hits = self._search(text, k=1)
                if hits:
                    return self.memory[hits[0]][1]
            except Exception:
                pass
    ...
    def _build_messages(self, text: str):
        ...
        # 任务自适应：MCQ 用选择题专用 system msg + reasoning；其他用标准分类器
        # MCQ 检测基于 train text 选项 stem（_detect_mcq_task），独立于 desc 生成状态
        mcq_mode = self._detect_mcq_task()
        ...
        # 任务自适应：三分支 system_msg
        #   - mcq_mode（无 desc，label 是 A/B/C/D 类）：用"答选择题"框架 + reasoning
        #   - desc 有 + 用 reasoning（DEV-best 变体）：用"分类器 + reasoning"
        #   - desc 有 + 不用 reasoning（加权最优，默认）：用"分类器直接输出 label"
        use_reasoning = True  # [dev-best + P0P1 合并变体] 全任务强制 reasoning；加权最优变体为 use_reasoning = mcq_mode
        if mcq_mode:
            system_msg = (
                "You are solving a multiple-choice question. "
                "The input inside <query> contains a question and its options; "
                "choose the correct option label from LABEL_SET. "
    ...
    def _detect_mcq_task(self) -> bool:
        """主信号：训练 text 含 ≥2 个选项 stem（A. / A: / A) / 1. / 一. / 甲. 等）。
        比"label 多为单字符"启发式更鲁棒：
        1) MCQ 用 apple/banana 等多字符 label 时，label 信号失效，但 text stem 仍在；
        2) 非 MCQ（OOD）偶然用 A/B/C/D label 时，label 信号误报，但 text 无选项 stem。
        FAQ 行 559 保证单任务内选项格式稳定，所以 ≥50% train 命中即可定性。"""
        if self._is_mcq_cache is not None:
            return self._is_mcq_cache
        if not self.memory:
            return False
        pat = re.compile(r'(?:^|\n)[ \t]*[A-Za-z0-9一二三四五六甲乙丙丁](?:\.|:|\))[ \t]+\S')
        sample = self.memory[:200]
        hit = 0
        for text, _ in sample:
            if len(pat.findall(text or "")) >= 2:
                hit += 1
        self._is_mcq_cache = (hit / max(len(sample), 1)) >= 0.5
        return self._is_mcq_cache

    def _should_use_descriptions(self) -> bool:
        """非 MCQ 任务才生成 label 描述。"""
        return bool(self._labels) and not self._detect_mcq_task()
    ...
    def predict(self, text: str) -> str:
        ...
        try:
            ...
            return self._retrieval_fallback(text)
