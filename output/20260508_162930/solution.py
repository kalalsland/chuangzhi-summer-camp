...
class MyHarness(Harness):
    ...
    def _build_messages(self, text: str):
        ...
        # 任务自适应：MCQ（无 desc）时 cap=3, n=12（4 labels × 3 = 12 max，教会 QA 任务框架）
        mcq_mode = not self._descriptions
        per_label_cap = 3 if mcq_mode else 2
        max_few_shot = 12 if mcq_mode else 8
        topk = self._search(text, k=32 if mcq_mode else 24)
        chosen, used = [], Counter()
        for d in topk:
            lab = self.memory[d][1]
            if used[lab] < per_label_cap:
                chosen.append(d)
                used[lab] += 1
            if len(chosen) >= max_few_shot:
                break
